...
if ( ! class_exists( '\PiwikTracker' ) ) {
	include_once plugin_dir_path( PSSTATS_ANALYTICS_FILE ) . 'app/vendor/psstats/psstats-php-tracker/PsstatsTracker.php';
}

class AjaxTracker extends \PsstatsTracker {
	...
	public function __construct( Settings $settings ) {
		...
		if ( $settings->get_global_option( 'track_api_endpoint' ) === 'restapi' ) {
			...
			$api_endpoint = $paths->get_tracker_api_url_in_psstats_dir();
		}
	...
	protected function sendRequest( $url, $method = 'GET', $data = null, $force = false ) {
		...
		// todo at some point we could think about including `psstats.php` here instead of doing an http request
