...
namespace WpPsstats\TrackingCode;
...
class TrackingCodeGenerator {
	...
	public function register_hooks() {
		add_action( 'psstats_site_synced', array( $this, 'update_tracking_code' ), $prio = 10, $args = 0 );
		add_action( 'psstats_tracking_settings_changed', array( $this, 'update_tracking_code' ), $prio = 10, $args = 0 );
	}
	...
	public function update_tracking_code() {
		...
		} elseif ( TrackingSettings::TRACK_MODE_TAGMANAGER === $track_mode && psstats_has_tag_manager() ) {
...
g.type=\'text/javascript\'; g.async=true; g.src="' . $container_url . '"; s.parentNode.insertBefore(g,s);
	...
	public function get_tracker_endpoint() {
		...
		if ( $this->settings->get_global_option( 'track_api_endpoint' ) === 'restapi' ) {
			...
			$tracker_endpoint = $paths->get_tracker_api_url_in_psstats_dir();
		}
	...
	public function get_js_endpoint() {
		...
		if ( $this->settings->get_global_option( 'track_js_endpoint' ) === 'restapi' ) {
			...
			$js_endpoint = plugins_url( 'app/psstats.js', PSSTATS_ANALYTICS_FILE );;
		} else {
			$js_endpoint = $paths->get_js_tracker_url_in_psstats_dir();
		}
...
g.type='text/javascript'; g.async=true; g.src=" . wp_json_encode( $js_endpoint ) . '; s.parentNode.insertBefore(g,s);';
		...
		$script = apply_filters( 'psstats_tracking_code_script', $script, $idsite );
		$script = apply_filters( 'psstats_tracking_code_noscript', $script, $idsite );
	...
	private function apply_user_tracking( $tracking_code ) {
		...
		$user_id_to_track = apply_filters( 'psstats_tracking_user_id', $user_id_to_track );
