...
class Capabilities {
	const KEY_NONE = 'none_psstats';

	/**
	 * @api
	 */
	const KEY_VIEW = 'view_psstats';

	/**
	 * @api
	 */
	const KEY_WRITE = 'write_psstats';

	/**
	 * @api
	 */
	const KEY_ADMIN = 'admin_psstats';

	/**
	 * @api
	 */
	const KEY_SUPERUSER = 'superuser_psstats';
	const KEY_STEALTH   = 'stealth_psstats';
	...
	public function map_meta_cap( $caps, $cap, $user_id, $args ) {
		...
		if ( Menu::CAP_NOT_EXISTS === $cap
			 && is_multisite()
			 && is_super_admin() ) {
			$caps[] = 'do_not_allow'; // prevent psstats-analytics submenu to be shown
		}
	...
	public function add_capabilities_to_user( $allcaps, $caps, $args, $user ) {
		if ( isset( $caps[0] ) ) {
			$cap_request = $caps[0];
			switch ( $cap_request ) {
				// ensure the Psstats capability inheritcance always works
