...
class Installer {
	...
	const OPTION_NAME_INSTALL_DATE = 'psstats-install-date';
	const OPTION_NAME_INSTALL_VERSION = 'psstats-install-version';
	...
	public function register_hooks() {
		add_action( 'activate_psstats', array( $this, 'install' ) );
	}
	...
	public static function is_intalled() {
		try {
			...
			return SettingsPiwik::isPsstatsInstalled();
	...
	public function set_psstats_url()
	{
		...
		$plugins_url = plugins_url( 'app', PSSTATS_ANALYTICS_FILE );
		// need to make sure to update plugins url if it changes eg if installed somewhere else or domain changes

		if ($psstats_url
		    && $plugins_url === $psstats_url
		    && parse_url($psstats_url, PHP_URL_SCHEME)
		    && parse_url($psstats_url, PHP_URL_HOST)
		) {
	...
	public static function get_db_infos( $default = array() ) {
		...
		$database = array(
			...
			'dbname' => DB_NAME,
			'charset' => $charset,
			'tables_prefix' => $wpdb->prefix . PSSTATS_DATABASE_PREFIX,
			'adapter' => 'WordPress',
		);
		if (!empty($socket)) {
