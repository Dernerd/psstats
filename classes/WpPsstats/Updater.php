...
use Piwik\Version;
use WpPsstats\Updater\UpdateInProgressException;
...
class Updater {
	const LOCK_NAME = 'psstats_updater';
	...
	public function get_plugins_requiring_update()
	{
		...
		$keys = [];
		$plugin_files = $GLOBALS['PSSTATS_PLUGIN_FILES'];
		if ( ! in_array( PSSTATS_ANALYTICS_FILE, $plugin_files, true ) ) {
			$plugin_files[] = PSSTATS_ANALYTICS_FILE;
			// making sure this plugin is in the list so when itself gets updated
			// it will execute the core updates
		}

		foreach ( $GLOBALS['PSSTATS_PLUGIN_FILES'] as $plugin_file ) {
	...
	public function update_if_needed() {
		...
		foreach ($plugins_requiring_update as $key => $plugin_version) {
			try {
				...
				$this->logger->log( 'Psstats-Update ist bereits in Arbeit');
	...
	public function update() {
		...
		if ( $this->load_plugin_functions() ) {
			...
			if ( ! empty( $plugin_data['Version'] )
				...
				// this allows us to see which versions of psstats the user was using before this update so we better understand
...
' . ServerFilesGenerator::getAllowHtaccessContent() . '
	...
	public static function lock()
	{
		...
		// we lock for 4 minutes. In case of major Psstats upgrades the upgrade may take much longer but it should be
	...
	private static function update_components() {
		...
		if (!empty($result['errors'])) {
			throw new \Exception('Fehler beim Aktualisieren von Komponenten: ' . implode(', ' , $result['errors']));
		}
