...
class Uninstaller {
	...
	public function uninstall( $should_remove_all_data ) {
		...
		do_action( 'psstats_uninstall', $should_remove_all_data );
	}

	public function uninstall_blog( $should_remove_all_data ) {
		$this->logger->log( 'Psstats deinstalliert jetzt blogId ' . get_current_blog_id() );
		...
		if ( $should_remove_all_data ) {
			$this->logger->log( 'Psstats ist gezwungen, alle Daten zu entfernen' );
		...
		do_action( 'psstats_uninstall_blog', $should_remove_all_data );

		$this->logger->log( 'Psstats hat die Deinstallation abgeschlossen ' . get_current_blog_id() );
	}

	public static function uninstall_options( $prefix ) {
		global $wpdb;

		self::make_logger()->log( 'Optionen mit Präfix entfernen ' . $prefix );
	...
	public static function uninstall_site_meta( $prefix ) {
		...
		if ( ! empty( $wpdb->sitemeta ) ) {
			// multisite
			self::make_logger()->log( 'Sitemeta mit Präfix entfernen ' . $prefix );
	...
	public static function uninstall_user_meta( $prefix ) {
		...
		if ( ! empty( $wpdb->usermeta ) ) {
			self::make_logger()->log( 'Usermeta mit Präfix entfernen ' . $prefix );
	...
	public function uninstall_multisite( $should_remove_all_data ) {
		...
		$this->logger->log( 'Psstats deinstalliert jetzt alle Blogs: ' . (int) $should_remove_all_data );
	...
	private function drop_tables() {
		...
		$installed_tables = $db_settings->get_installed_psstats_tables();
		$this->logger->log( sprintf( 'Psstats löscht jetzt %s psstats-Tabellen', count( $installed_tables ) ) );
