...
class Settings {
	...
	/**
	 * This feature can be used to read data from psstats tables without needing to bootstrap psstats
	 *
	...
	public function prefix_table_name( $table_name_to_prefix ) {
		...
		return $wpdb->prefix . PSSTATS_DATABASE_PREFIX . $table_name_to_prefix;
	}

	public function get_installed_psstats_tables() {
		global $wpdb;

		$table_names = array();
		$tables      = $wpdb->get_results( 'SHOW TABLES LIKE "' . $wpdb->prefix . str_replace( '_', '\_', PSSTATS_DATABASE_PREFIX ) . '%"', ARRAY_N );
		...
		$table_names_to_look_for = apply_filters( 'psstats_install_tables', $table_names_to_look_for );
