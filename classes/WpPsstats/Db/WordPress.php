...
class WordPress extends Mysqli {
	...
	private function before_execute_query( $wpdb, $sql ) {
		if ( ! $wpdb->suppress_errors ) {
			if ( defined( 'PSSTATS_SUPPRESS_DB_ERRORS' ) ) {
				// allow users to always suppress or never suppress
				if ( PSSTATS_SUPPRESS_DB_ERRORS === true ) {
			...
			if ( ( stripos( $sql, '/* WP IGNORE ERROR */' ) !== false  )
				...
				// SELECT 1 FROM wp_psstats_logtmpsegment1cc77bce7a13181081e44ea6ffc0a9fd LIMIT 1 => runs to detect if temp table exists or not and regularly the query fails which is expected
	...
	public function fetchOne( $sql, $bind = array() ) {
		...
		if ( $value === null ) {
			return false; // make sure to behave same way as psstats
		}
