...
return array(
	...
	'path.misc.user' => function () {
		$paths = new \WpPsstats\Paths();
		return $paths->get_relative_dir_to_psstats($paths->get_upload_base_dir()) . '/';
	},
	...
	'TagManagerContainerStorageDir' => function () {
		...
		$paths = new \WpPsstats\Paths();
		return rtrim('/'. $paths->get_relative_dir_to_psstats($paths->get_upload_base_dir().'/'), '/');
	},
	...
	\Piwik\Config::class => DI\decorate(function ($previous) {
		...
		$previous->database = \WpPsstats\Installer::get_db_infos($database);

		$paths = new Paths();
		if ( file_exists( $paths->get_config_ini_path() ) ) {
			$general = $previous->General;

			if (defined('PSSTATS_TRIGGER_BROWSER_ARCHIVING')) {
				$general['enable_browser_archiving_triggering'] = (int) PSSTATS_TRIGGER_BROWSER_ARCHIVING;
			}

			$psstats_salt_key = Settings::OPTION_PREFIX . 'psstats_salt';
			$psstats_salt = get_option($psstats_salt_key); // needs to be per site!
			if (!$psstats_salt) {
				$psstats_salt = \Piwik\Common::getRandomString(32);
				update_option($psstats_salt_key, $psstats_salt, true);
			}

			$general['salt'] = $psstats_salt;
			...
			if (empty($GLOBALS['PSSTATS_SWITCH_BLOG_SET_UP'])) {
			    // only execute it once since we might init this several times...
                $GLOBALS['PSSTATS_SWITCH_BLOG_SET_UP'] = true;
	...
	'Piwik\Mail\Transport' => DI\autowire('WpPsstats\Email'),
	'Piwik\Plugins\CustomJsTracker\TrackerUpdater' => DI\decorate(function ($previous) {
		...
		$paths = new Paths();
		$dir = $paths->get_psstats_js_upload_path();
		...
		foreach ($checks as $index => $check) {
			if ($check && is_object($check)) {
				...
				if ($class_name === 'Piwik\Plugins\Diagnostics\Diagnostic\ForceSSLCheck'
					...
					|| $class_name === 'Piwik\Plugins\Diagnostics\Diagnostic\RequiredPrivateDirectories' // it doesn't resolve config path correctly as it is outside psstats dir etc
	...
	'observers.global' => DI\add(array(
		array('FrontController.modifyErrorPage', DI\value(function (&$result, $ex) {
			if (!empty($ex) && is_object($ex) && $ex instanceof \Piwik\Exception\NoWebsiteFoundException) {
				...
				// then work
				$sync = new \WpPsstats\Site\Sync(new Settings());
		...
		array('Db.getDatabaseConfig', DI\value(function (&$config) {
			...
			include_once plugin_dir_path(PSSTATS_ANALYTICS_FILE ) . 'classes/WpPsstats/Db/WordPress.php';
		})),
