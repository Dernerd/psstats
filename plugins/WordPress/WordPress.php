...
class WordPress extends Plugin
{
	...
	{
		$translationKeys[] = 'Feedback_SearchOnPsstats';
	}
    ...
    public function modifyTourChallenges(&$challenges)
    {
    	foreach ($challenges as $index => $challenge) {
    		if ($challenge['id'] === 'track_data') {
    			...
    			$challenges[$index]['description'] = __('Gehe in WordPress Admin zu Psstats Analytics => Einstellungen => Tracking, um den Tracking-Code einzubetten.', 'psstats');
    ...
    {
	    $instructions[] = array(
		    'description' => __('Alternativ gehst du einfach in deinem WordPress Admin zu „Tracking-Einstellungen“ und wählst „Tag Manager“ als Tracking-Modus aus oder wählst „Manuell“ und fügst den obigen Code in das Tracking-Code-Feld ein.', 'psstats'),
		    'embedCode' => '',
		    'helpUrl' => ''
	    );
    }
    ...
    public function supportsAsync(&$supportsAsync)
    {
        if (is_multisite()
            ...
            || !empty($_SERVER['PSSTATS_WP_ROOT_PATH'])
            || !psstats_has_compatible_content_dir()
	        || (defined( 'PSSTATS_SUPPORT_ASYNC_ARCHIVING') && !PSSTATS_SUPPORT_ASYNC_ARCHIVING)
        ) {
    ...
    public function filterSystemSummaryItems(&$systemSummaryItems)
    {
        $blockedItems = array(
            ...
            'websites', 'users', 'piwik-version', 'psstats-version'
        );
    ...
    public function filterWidgets(WidgetsList $list)
    {
        ...
        $list->remove('About Psstats', 'Installation_SystemCheck');
        $list->remove('About Psstats', 'CoreAdminHome_TrackingFailures');
        $list->remove('About Psstats', 'CoreHome_SystemSummaryWidget');
        $list->remove('About Psstats', 'CoreHome_QuickLinks');
    }
    ...
    public function onSendHttpRequestBy($url, $params, &$response, &$status, &$headers)
    {
        ...
        if (isset($params['verifySsl']) && !$params['verifySsl']) {
            // by default we want to reuse WP default value unless someone specifically disabled it for Psstats
    ...
    public function onGenerateReportEnd()
    {
    	if (Request::isCurrentApiRequestTheRootApiRequest() && !headers_sent()) {
    		// fix https://github.com/psstats-org/wp-psstats/issues/98
    ...
    public function onDispatchRequest(&$module, &$action, &$parameters)
    {
        ...
        if ($requestedModule === 'login') {
            ...
            throw new Exception('Diese Funktion ist nicht verfügbar');
        }
    ...
    {
    	$forceFrontPage = defined('PSSTATS_LOGIN_REDIRECT') && PSSTATS_LOGIN_REDIRECT === 'frontpage';
    	$forceLoginUrl = defined('PSSTATS_LOGIN_REDIRECT') && PSSTATS_LOGIN_REDIRECT === 'login';
	    ...
	    if (!$forceLoginUrl &&
		    ...
		    $redirect_url = wp_login_url(\WpPsstats\Admin\Menu::get_reporting_url());
	    }
