...
/**
 * PsstatsTracker implements the Psstats Tracking Web API.
 *
 * For more information, see: https://github.com/psstats-org/psstats-php-tracker/
 *
 * @package PsstatsTracker
 * @api
 */
class PsstatsTracker
{
    /**
     * Psstats base URL, for example http://example.org/psstats/
     * Must be set before using the class by calling
     * PsstatsTracker::$URL = 'http://yourwebsite.org/psstats/';
     *
     * @var string
     */
    static public $URL = '';
    ...
    /**
     * Builds a PsstatsTracker object, used to track visits, pages and Goal conversions
     * for a specific website, by using the Psstats Tracking API.
     *
     * @param int $idSite Id site to be tracked
     * @param string $apiUrl "http://example.org/psstats/" or "http://psstats.example.org/"
     *                         If set, will overwrite PsstatsTracker::$URL
     */
    ...
    /**
     * By default, Psstats expects utf-8 encoded values, for example
     * for the page URL parameter values, Page Title, etc.
     * It is recommended to only send UTF-8 data to Psstats.
     ...
     * psstatsTracker.getAttributionInfo() and that you have JSON encoded via JSON2.stringify()
     *
     ...
     * @return $this
     * @throws Exception
     * @see function getAttributionInfo() in https://github.com/psstats-org/psstats/blob/master/js/psstats.js
     */
     ...
     *
     * If scope is 'visit', it will attempt to read the value set in the first party cookie created by Psstats Tracker
     *  ($_COOKIE array).
     *
     ...
     * @see psstats.js getCustomVariable()
     */
     ...
     * plugin that is not shipped with Psstats itself. Please note that custom parameters are cleared after each
     * tracking request.
     *
    ...
    /**
     * Sets the country of the visitor. If not used, Psstats will try to find the country
    ...
    /**
     * Sets the region of the visitor. If not used, Psstats may try to find the region
    ...
    /**
     * Sets the city of the visitor. If not used, Psstats may try to find the city
     * using the visitor's IP address (if configured to do so).
     *
     * Allowed only for Admin/Super User, must be used along with setTokenAuth().
     * @param string $city
    ...
    /**
     * Sets the latitude of the visitor. If not used, Psstats may try to find the visitor's
    ...
    /**
     * Sets the longitude of the visitor. If not used, Psstats may try to find the visitor's
    ...
    /**
     * If image response is disabled Psstats will respond with a HTTP 204 header instead of responding with a gif.
     */
    ...
    {
        // NOTE: If the cookie name is changed, we must also update the method in psstats.js with the same name.
        $hash = substr(
            sha1(
     ...
     * All revenues (grandTotal, subTotal, tax, shipping, discount) will be individually summed and reported in Psstats reports.
     * Only the parameters $orderId and $grandTotal are required.
     *
     * @param string|int $orderId (required) Unique Order ID.
     *                This will be used to count this order only once in the event the order page is reloaded several times.
     *                orderId must be unique for each transaction, even on different days, or the transaction will not be recorded by Psstats.
     ...
     *
     * Tracking Product/Category page views will allow Psstats to report on Product & Categories
    ...
    /**
     * Force the separator for decimal point to be a dot. See https://github.com/psstats-org/psstats/issues/6435
     ...
     *
     * @see doTrackPageView()
     * @param string $documentTitle Page view name as it will appear in Psstats reports
     * @return string URL to psstats.php with all parameters set to track the pageview
     */
     ...
     * @param int $idGoal Id Goal to record a conversion
     * @param float $revenue Revenue for this conversion
     * @return string URL to psstats.php with all parameters set to track the goal conversion
     */
     ...
     * @param string $actionUrl URL of the download or outlink
     * @param string $actionType Type of the action: 'download' or 'link'
     * @return string URL to psstats.php with all parameters set to track an action
     */
     ...
     * By default Psstats will track requests for the "current datetime" but this function allows you
    ...
    /**
     * Forces Psstats to create a new visit for the tracking request.
     *
     * By default, Psstats will create a new visit if the last request by this user was more than 30 minutes ago.
    ...
    /**
     * Hash function used internally by Psstats to hash a User ID into the Visitor ID.
     *
     ...
     *
     * Rather than letting Psstats attribute the user with a heuristic based on IP and other user fingeprinting attributes,
    ...
    /**
     * If the user initiating the request has the Psstats first party cookie,
...
didn't change any existing VisitorId value */
     ...
     * @see psstats.js getAttributionInfo()
     */
     ...
     * This is reported in "List of plugins" report in Psstats.
     *
     * @param bool $bool
     * @return $this
     */
    ...
    /**
     * By default, PsstatsTracker will read first party cookies
     ...
     * from Psstats. Defaults to 600 seconds.
     */
    ...
    /**
     * If a proxy is needed to look up the address of the Psstats site, set it with this
    ...
    /**
     * Returns the base URL for the Psstats server.
     */
    protected function getBaseUrl()
    {
        if (empty(self::$URL)) {
            throw new Exception(
                'You must first set the Psstats Tracker URL by calling
                 PsstatsTracker::$URL = \'http://your-website.org/psstats/\';'
            );
        }
        if (strpos(self::$URL, '/psstats.php') === false
            && strpos(self::$URL, '/proxy-psstats.php') === false
        ) {
            self::$URL = rtrim(self::$URL, '/');
            self::$URL .= '/psstats.php';
        }
    ...
    protected function getCookieMatchingName($name)
    {
        ...
        // Psstats cookie names use dots separators in psstats.js,
    ...
    /**
     * Sets the first party cookies as would the psstats.js
     ...
     *
     * This replicates the psstats.js tracker algorithms for consistency and better accuracy.
     *
     * @param $cookieName
     * @param $cookieValue
     * @param $cookieTTL
 ...
 * @return string
 */
function Psstats_getUrlTrackPageView($idSite, $documentTitle = '')
{
    $tracker = new PsstatsTracker($idSite);
 ...
 * @return string
 */
function Psstats_getUrlTrackGoal($idSite, $idGoal, $revenue = 0.0)
{
    $tracker = new PsstatsTracker($idSite);
