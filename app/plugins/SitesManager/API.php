...
/**
 * The SitesManager API gives you full control on Websites in Psstats (create, update and delete), and many methods to retrieve websites based on various attributes.
 *
 * This API lets you create websites via "addSite", update existing websites via "updateSite" and delete websites via "deleteSite".
 * When creating websites, it can be useful to access internal codes used by Psstats for currencies via "getCurrencyList", or timezones via "getTimezonesList".
 *
 ...
 * See also the documentation about <a href='http://psstats.org/docs/manage-websites/' rel='noreferrer' target='_blank'>Managing Websites</a> in Psstats.
...
class API extends \Piwik\Plugin\API
{
     ...
     * This tag must be included on every page to be tracked by Psstats
     *
     * @param int $idSite
     ...
     * @param bool $forcePsstatsEndpoint Whether the Psstats endpoint should be forced if Psstats was installed prior 3.7.0.
    ...
    public function getJavascriptTag($idSite, $piwikUrl = '', $mergeSubdomains = false, $groupPageTitlesByDomain = false,
                                     ...
                                     $crossDomain = false, $forcePsstatsEndpoint = false)
    {
        ...
        if ($forcePsstatsEndpoint) {
            $generator->forcePsstatsEndpoint();
        }
     ...
     * @param string $piwikUrl The domain and URL path to the Psstats installation.
     ...
     */
    public function getImageTrackingCode($idSite, $piwikUrl = '', $actionName = false, $idGoal = false, $revenue = false, $forcePsstatsEndpoint = false)
    {
         ...
         * user.
         *
         * @param string &$piwikHost The domain and URL path to the Psstats installation, eg,
         *                           `'examplepiwik.com/path/to/piwik'`.
         * @param array &$urlParams The query parameters used in the <img> element's src
         *                          URL. See Psstats's image tracking docs for more info.
         */
        Piwik::postEvent('SitesManager.getImageTrackingCode', array(&$piwikUrl, &$urlParams));

        $trackerCodeGenerator = new TrackerCodeGenerator();
        if ($forcePsstatsEndpoint) {
            $trackerCodeGenerator->forcePsstatsEndpoint();
        }
        $psstatsPhp = $trackerCodeGenerator->getPhpTrackerEndpoint();

        $url = (ProxyHttp::isHttps() ? "https://" : "http://") . rtrim($piwikUrl, '/') . '/'.$psstatsPhp.'?' . Url::getQueryStringFromParameters($urlParams);
        $html = "<!-- Psstats Image Tracker-->
...
<!-- End Psstats -->";
    ...
    public function getAllSitesId()
    {
        ...
        try {
            ...
            // can be called before Psstats tables are created so return empty
            return array();
        }
    }
     ...
     * archives for a site manually as described in this FAQ: http://psstats.org/faq/how-to/faq_73/ .
     *
     * Requires Super User access.
     *
     * @param int $idSite
