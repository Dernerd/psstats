...
function submitForm(e, form) {
    ...
    if (typeof parent === 'object' && typeof parent.postMessage !== 'undefined') {
        addEventListener(window, 'message', function(e) {
            ...
            if (!hasOptOutChangeWorkedThroughPostMessage) {
                ...
                // this might be IF eg the Psstats instance trackerUrl on the page does not match the Psstats instance optOut url...
                showWarningIfHttp();
            }
        });
    ...
    // Fire off a request to Psstats in the background, which will try to set the third-party cookie.
...
function showWarningIfCookiesDisabled() {
    if (navigator && !navigator.cookieEnabled) {
        // Error condition: cookies disabled and Psstats not configured to opt the user out by default = they can't opt out
