 ...
 * See also the documentation about <a href='http://psstats.org/docs/manage-users/' rel='noreferrer' target='_blank'>Managing Users</a> in Psstats.
 */
class API extends \Piwik\Plugin\API
{
    ...
    public function getUsersPlusRole($idSite, $limit = null, $offset = 0, $filter_search = null, $filter_access = null)
    {
        if (!$this->isUserHasAdminAccessTo($idSite)) {
            ...
            if ($offset > 1) {
                Common::sendHeader('X-Psstats-Total-Results: 1');
                return [];
            }
            ...
            if ($loginsToLimit !== null && empty($loginsToLimit)) {
                ...
                Common::sendHeader('X-Psstats-Total-Results: 0');
                return [];
        ...
        Common::sendHeader('X-Psstats-Total-Results: ' . $totalResults);
        return $users;
    }
    ...
    public function getSitesAccessForUser($userLogin, $limit = null, $offset = 0, $filter_search = null, $filter_access = null)
    {
        ...
        Common::sendHeader('X-Psstats-Total-Results: ' . $totalResults);
        if ($hasAccessToAny) {
            Common::sendHeader('X-Psstats-Has-Some: 1');
        }
        return $sites;
    }
