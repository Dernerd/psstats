...
class FormDatabaseSetup extends QuickForm2
{
    ...
    function init()
    {
        ...
        foreach ($defaultsEnvironment as $name) {
            ...
            $envNamePsstats = 'PSSTATS_' . $envName;
            if (getenv($envNamePsstats)) {
                $defaults[$name] = getenv($envNamePsstats);
 ...
 *
 * The following privileges are required for Psstats to run:
 * - CREATE
 * - ALTER
 * - SELECT
 * - INSERT
...
class Rule_checkUserPrivileges extends HTML_QuickForm2_Rule
{
    ...
    /**
     * Returns an array describing the database privileges required for Psstats to run. The
    ...
    /**
     * Returns a string description of the database privileges required for Psstats to run.
     *
     * @return string
     */
