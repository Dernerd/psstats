...
class GenerateIntl extends ConsoleCommand
{
    ...
    {
        $psstatsLanguages = \Piwik\Plugins\LanguagesManager\API::getInstance()->getAvailableLanguages();

        if ($input->getOption('language')) {
            $psstatsLanguages = [$input->getOption('language')];
        }
        ...
        foreach ($psstatsLanguages AS $langCode) {
    ...
    protected function checkCurrencies(OutputInterface $output)
    {
        ...
        $psstatsCurrencies = array_keys(include $file);

        $missing = array_diff($cldrCurrencies, $psstatsCurrencies);
        $additional = array_diff($psstatsCurrencies, $cldrCurrencies);
