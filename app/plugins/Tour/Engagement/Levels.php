...
class Levels
{
    ...
    public function getLevels()
    {
        ...
        $levels = array(
            0 => Piwik::translate('Tour_PsstatsBeginner'),
            5 => Piwik::translate('Tour_PsstatsIntermediate'),
        );

        if ($numChallengesTotal > 10) {
            // the number of challenges varies from Psstats to Psstats depending on activated plugins and activated
            // features. Therefore we may remove some levels if there aren't too many challenges available.
            $levels[10] = Piwik::translate('Tour_PsstatsTalent');
        }

        if ($numChallengesTotal > 15) {
            $levels[15] = Piwik::translate('Tour_PsstatsProfessional');
        }

        $levels[$numChallengesTotal] = Piwik::translate('Tour_PsstatsExpert');
