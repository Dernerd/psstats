...
class PsstatsTag extends BaseTag
{
    const ID = 'Psstats';
    const PARAM_PSSTATS_CONFIG = 'psstatsConfig';
    ...
    public function getParameters()
    {
        ...
        return array(
            $this->makeSetting(self::PARAM_PSSTATS_CONFIG, '', FieldConfig::TYPE_STRING, function (FieldConfig $field) {
                $field->title = 'Psstats Configuration';
                $field->description = 'Assign a Psstats configuration in order to track data into a specific site.';
                $field->customUiControlTemplateFile = self::FIELD_TEMPLATE_VARIABLE_TYPE;
                $field->uiControlAttributes = array('variableType' => 'PsstatsConfiguration');
                $field->validators[] = new NotEmpty();
            }),
            $trackingType,
    ...
    public function loadTemplate($context, $entity)
    {
        ...
        $bundleTrackerEnabled = !isset($entity['parameters']['psstatsConfig']['parameters']['bundleTracker'])
                             || !empty($entity['parameters']['psstatsConfig']['parameters']['bundleTracker']);
        if ($template && $bundleTrackerEnabled) {
            ...
            if (!$tracker) {
                $tracker = @file_get_contents(PIWIK_DOCUMENT_ROOT . '/psstats.js');
            }
            if (!$tracker) {
