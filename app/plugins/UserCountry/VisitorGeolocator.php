...
use Psstats\Cache\Cache;
use Psstats\Cache\Transient;
use Piwik\Common;
    ...
    public static function getDefaultLocationCache()
    {
        if (self::$defaultLocationCache === null) {
            if (class_exists('\Piwik\Cache\Transient')) {
                // during the oneclickupdate from 3.x => greater, this class will be loaded, so we have to use it instead of the Psstats namespaced one
