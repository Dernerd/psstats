...
abstract class GeneratePluginBase extends ConsoleCommand
{
    ...
    protected function checkAndUpdateRequiredPiwikVersion($pluginName, OutputInterface $output)
    {
        ...
        if (!empty($pluginJson['require']['piwik'])) {
            $pluginJson['require']['psstats'] = $pluginJson['require']['piwik'];
            unset($pluginJson['require']['piwik']);
        }

        if (!empty($pluginJson['require']['psstats'])) {
            $requiredVersion = trim($pluginJson['require']['psstats']);
        ...
        $pluginJson['require']['psstats'] = $newRequiredVersion;
    ...
    protected function throwErrorIfNotGitInstalled()
    {
        if (!SettingsPiwik::isGitDeployment()) {
            $url = 'https://developer.psstats.org/guides/getting-started-part-1';
            throw new NotGitInstalledException("This development feature requires Psstats to be checked out from git. For more information please visit {$url}.");
        }
    }
}
