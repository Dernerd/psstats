...
{
    /**
     * whether psstats.js|php should be forced over piwik.js|php
     * @var bool
     */
    private $shouldForcePsstatsEndpoint = false;

    public function forcePsstatsEndpoint()
    {
        $this->shouldForcePsstatsEndpoint = true;
    }
    ...
    public function generate(
        ...
        $codeImpl = array(
            ...
            'psstatsJsFilename'        => $this->getJsTrackerEndpoint(),
            'psstatsPhpFilename'       => $this->getPhpTrackerEndpoint(),
        );
    ...
    {
        if ($this->shouldForcePsstatsEndpoint) {
            return false;
        }

        // only since 3.7.0 we use the default psstats.js|php... for all other installs we need to keep BC
        return DbHelper::wasPsstatsInstalledBeforeVersion('3.7.0-b1');
    }
