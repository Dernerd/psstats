...
class Dependency
{
    ...
    public function getMissingDependencies($requires)
    {
        ...
        foreach ($requires as $name => $requiredVersion) {
            ...
            if (in_array(strtolower($name), ['piwik', 'psstats'])) {
                $requiredVersion = $this->markPluginsWithoutUpperBoundPsstatsRequirementAsIncompatible($requiredVersion);
            }
    ...
    /**
     * Upon Psstats 4 we require a lower and upper version bound for Psstats to be set in plugin.json
     * If that is not the case we assume the plugin not to be compatible with Psstats 4
     *
     * @param string $requiredVersion
     * @return string
     */
    private function markPluginsWithoutUpperBoundPsstatsRequirementAsIncompatible($requiredVersion)
    {
    ...
    public function hasDependencyToDisabledPlugin($requires)
    {
        ...
        foreach ($requires as $name => $requiredVersion) {
            ...
            $isPluginRequire = !in_array($nameLower, array('piwik', 'php', 'psstats'));
            if ($isPluginRequire) {
