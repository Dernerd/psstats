...
class Manager
{
    ...
    {
        $envDirs = getenv('PSSTATS_PLUGIN_DIRS');
        if (!empty($envDirs)) {
            ...
            foreach ($envDirs as $envDir) {
                ...
                $GLOBALS['PSSTATS_PLUGIN_DIRS'][] = array(
                    ...
                    'webrootDirRelativeToPsstats' => isset($envDir[1]) ? $envDir[1] : null,
                );
            }
        }

        if (!empty($GLOBALS['PSSTATS_PLUGIN_DIRS'])) {
            foreach ($GLOBALS['PSSTATS_PLUGIN_DIRS'] as $pluginDir => &$settings) {
                if (!isset($settings['pluginsPathAbsolute'])) {
                    throw new \Exception('Missing "pluginsPathAbsolute" configuration for plugin dir');
                }
                if (!isset($settings['webrootDirRelativeToPsstats'])) {
                    throw new \Exception('Missing "webrootDirRelativeToPsstats" configuration for plugin dir');
                }
            }
        ...
        $envCopyDir =  getenv('PSSTATS_PLUGIN_COPY_DIR');
        if (!empty($envCopyDir)) {
            $GLOBALS['PSSTATS_PLUGIN_COPY_DIR'] = $envCopyDir;
        }

        if (!empty($GLOBALS['PSSTATS_PLUGIN_COPY_DIR'])
            && !in_array($GLOBALS['PSSTATS_PLUGIN_COPY_DIR'], self::getPluginsDirectories())
        ) {
            throw new \Exception('"PSSTATS_PLUGIN_COPY_DIR" dir must be one of "PSSTATS_PLUGIN_DIRS" directories');
        }
    }

    /**
     * Registers a new autoloader to support the loading of Psstats plugin classes when the plugins are installed
     * outside the Psstats plugins folder.
    ...
    public static function getAlternativeWebRootDirectories()
    {
        ...
        if (!empty($GLOBALS['PSSTATS_PLUGIN_DIRS'])) {
            foreach ($GLOBALS['PSSTATS_PLUGIN_DIRS'] as $pluginDir) {
                $absolute = rtrim($pluginDir['pluginsPathAbsolute'], '/') . '/';
                $relative = rtrim($pluginDir['webrootDirRelativeToPsstats'], '/') . '/';
    ...
    /**
     * Returns the path to the directory where core plugins are located. Please note since Psstats 3.9
     * plugins may also be located in other directories and therefore this method has been deprecated.
     * @internal since Psstats 3.9.0 use {@link (getPluginsDirectories())} or {@link getPluginDirectory($pluginName)} instead
     * @return string
     */
    ...
    public function uninstallPlugin($pluginName)
    {
        if ($this->isPluginLoaded($pluginName)) {
            throw new \Exception("To uninstall the plugin $pluginName, first disable it in Psstats > Settings > Plugins");
        }
    ...
    public static function deletePluginFromFilesystem($plugin)
    {
        ...
        if (strpos($pluginDir, PIWIK_INCLUDE_PATH) === 0) {
            // only delete files for plugins within psstats directory...
    ...
    private function reloadActivatedPlugin($pluginName, $pluginsToPostPendingEventsTo)
    {
        ...
        if ($newPlugin->isPremiumFeature()
            ...
            if ($cache->contains($cacheKey)) {
                ...
                // prevent requesting license info during a tracker request see https://github.com/psstats-org/psstats/issues/14401
    ...
    private function isPluginBogus($pluginName)
    {
        $bogusPlugins = array(
            ...
            'wp-optimize', // a WP plugin that has a plugin.json file but is not a psstats plugin
        );
    ...
    private function savePluginTime($timingName, $pluginName)
    {
        ...
        try {
            ...
            // we ignore any error while Psstats is not installed yet. refs #16741
        }
    }
