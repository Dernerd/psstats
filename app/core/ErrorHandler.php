...
class ErrorHandler
{
    ...
    {
        return sprintf(
            "%s(%d): %s - %s - Psstats " . (class_exists('Piwik\Version') ? Version::VERSION : '') . " - Please report this message in the Psstats forums: https://forum.psstats.org (please do a search first as it might have been reported already)",
            $errfile,
            $errline,
    ...
    private static function getHtmlMessage($errno, $errstr, $errfile, $errline, $trace)
    {
        ...
        $html = "<p>There is an error. Please report the message (Psstats " . (class_exists('Piwik\Version') ? Version::VERSION : '') . ")
        and full backtrace in the <a target='_blank' rel='noreferrer noopener' href='https://forum.psstats.org'>Psstats forums</a> (please do a search first as it might have been reported already!).</p>";
