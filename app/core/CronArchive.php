...
class CronArchive
{
    ...
    public function run()
    {
        ...
        $this->logger->info("Starting Psstats reports archiving...");
        ...
        while (true) {
            ...
            if (!Process::isMethodDisabled('getmypid') && !Process::isMethodDisabled('ignore_user_abort')) {
                // see https://github.com/psstats-org/wp-psstats/issues/163
                flush();
            }
    ...
    private function logNetworkError($url, $response)
    {
        ...
        if (empty($response)) {
            ...
            if($this->supportsAsync) {
                ...
                $message .= " For more information and the error message please check your web server's error Log file. As this core:archive command triggers PHP processes over HTTP, you can find the error message in your Psstats's web server error logs. ";
            }
        } else {
    ...
    private function logInitInfo()
    {
        ...
        $this->logger->info("Running Psstats " . Version::VERSION . " as Super User");
    }
    ...
    private function logArchiveTimeoutInfo()
    {
        ...
        $this->logger->info("- Reports for today will be processed at most every " . $this->todayArchiveTimeToLive
            . " seconds. You can change this value in Psstats UI > Settings > General Settings.");
        ...
        if ($this->maxConcurrentArchivers && $cliMulti->supportsAsync()) {
            ...
            foreach ($processes as $process) {
                if (strpos($process, ' core:archive') !== false &&
                    ...
                    (!$instanceId
                        || strpos($process, '--psstats-domain=' . $instanceId) !== false
                        || strpos($process, '--psstats-domain="' . $instanceId . '"') !== false
                        || strpos($process, '--psstats-domain=\'' . $instanceId . "'") !== false
