...
use Piwik\Common;
use Psstats\Ini\IniReader;
use Psstats\Ini\IniReadingException;
use Psstats\Ini\IniWriter;
use Piwik\Piwik;
...
class IniFileChain
{
    ...
    public function reload($defaultSettingsFiles = array(), $userSettingsFile = null)
    {
        ...
        if (!empty($GLOBALS['PSSTATS_MODIFY_CONFIG_SETTINGS']) && !empty($this->mergedSettings)) {
            $this->mergedSettings = call_user_func($GLOBALS['PSSTATS_MODIFY_CONFIG_SETTINGS'], $this->mergedSettings);
        }
