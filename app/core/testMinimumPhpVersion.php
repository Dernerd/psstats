 ...
 * It checks the minimum PHP version required to run Psstats.
...
if ($minimumPhpInvalid) {
    $piwik_errorMessage .= "<p><strong>To run Psstats you need at least PHP version $piwik_minimumPHPVersion</strong></p>
				<p>Unfortunately it seems your webserver is using PHP version $piwik_currentPHPVersion. </p>
				<p>Please try to update your PHP version, Psstats is really worth it! Nowadays most web hosts
				support PHP $piwik_minimumPHPVersion.</p>";
} else {
    if (!extension_loaded('session')) {
        $piwik_errorMessage .= "<p><strong>Psstats and Zend_Session require the session extension</strong></p>
					<p>It appears your PHP was compiled with <pre>--disable-session</pre>.
					To enjoy Psstats, you need PHP compiled without that configure option.</p>";
    }

    if (!function_exists('ini_set')) {
        $piwik_errorMessage .= "<p><strong>Psstats and Zend_Session require the <code>ini_set()</code> function</strong></p>
					<p>It appears your PHP has disabled this function.
					To enjoy Psstats, you need remove <pre>ini_set</pre> from your <pre>disable_functions</pre> directive in php.ini, and restart your webserver.</p>";
    }

    if (ini_get('mbstring.func_overload')) {
        $piwik_errorMessage .= "<p><strong>Psstats does not work when PHP is configured with <pre>mbstring.func_overload = " . ini_get('mbstring.func_overload') . "</pre></strong></p>
					<p>It appears your mbstring extension in PHP is configured to override string functions.
					To enjoy Psstats, you need to modify php.ini <pre>mbstring.func_overload = 0</pre>, and restart your webserver.</p>";
    }

    if (!function_exists('json_encode')) {
        $piwik_errorMessage .= "<p><strong>Psstats requires the php-json extension which provides the functions <code>json_encode()</code> and <code>json_decode()</code></strong></p>
					<p>It appears your PHP has not yet installed the php-json extension.
					To use Psstats, please ask your web host to install php-json or install it yourself, for example on debian system: <code>sudo apt-get install php-json</code>. <br/>Then restart your webserver and refresh this page.</p>";
    }

    if (!file_exists(PIWIK_VENDOR_PATH . '/autoload.php')) {
        $composerInstall = "In the psstats directory, run in the command line the following (eg. via ssh): \n\n"
        ...
        if (DIRECTORY_SEPARATOR === '\\' /* ::isWindows() */) {
            $composerInstall = "Download and run <a href=\"https://getcomposer.org/Composer-Setup.exe\"><b>Composer-Setup.exe</b></a>, it will install the latest Composer version and set up your PATH so that you can just call composer from any directory in your command line. "
                . " <br>Then run this command in a terminal in the psstats directory: <br> $ php composer.phar install ";
        }
...
define('PAGE_TITLE_WHEN_ERROR', 'Psstats &rsaquo; Error');
...
if (!function_exists('Piwik_GetErrorMessagePage')) {
    ...
    {
        error_log(sprintf("Error in Psstats: %s", str_replace("\n", " ", strip_tags($message))));
        ...
        $content = '<h2>' . $message . '</h2>
                    <p>'
            . $optionalLinkBack
            . ' | <a href="index.php">Go to Psstats</a>'
            . '</p>'
