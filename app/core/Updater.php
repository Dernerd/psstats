...
class Updater
{
    ...
    const OPTION_KEY_PSSTATS_UPDATE_HISTORY = 'PsstatsUpdateHistory';
        ...
        foreach ($this->componentsWithUpdateFile as $componentName => $componentUpdateInfo) {
            foreach ($componentUpdateInfo as $file => $fileVersion) {
                ...
                if (!class_exists($className, false)) {
                    // throwing an error here causes Psstats to show the safe mode instead of showing an exception fatal only
                    // that makes it possible to deactivate / uninstall a broken plugin to recover Psstats directly
    ...
    public function updateComponents($componentsWithUpdateFile)
    {
        ...
        try {
            $history = Option::get(self::OPTION_KEY_PSSTATS_UPDATE_HISTORY);
            ...
            if (!empty($previousVersion) && !in_array($previousVersion, $history, true)) {
                ...
                Option::set(self::OPTION_KEY_PSSTATS_UPDATE_HISTORY, implode(',', $history));
            }
        } catch (\Exception $e) {
            // case when the option table is not yet created (before 0.2.10)
        }
