...
; you will lose your changes when you upgrade Psstats.
...
; if charset is set to utf8, Psstats will ensure that it is storing its data using UTF8 charset.
; it will add a sql query SET at each page view.
; Psstats should work correctly without this setting but we recommend to have a charset set.
charset = utf8
...
; if Psstats is available at http://localhost/dev/psstats/ replace @REQUEST_URI@ with /dev/psstats/
...
logger_file_path = tmp/logs/psstats.log
...
; 'redis' will cache on a Redis server, use this if you are running Psstats with multiple servers. Further configuration in [RedisCache] is needed
...
; If set to 1, all requests to psstats.php will be forced to be 'new visitors'
...
; Note that for quick debugging, instead of using below setting, you can add `&disable_merged_assets=1` to the Psstats URL
...
; year and range periods are disabled by default, to ensure optimal performance for high traffic Psstats instances
; if you set it to 1 and want the Unique Visitors to be re-processed for reports in the past, drop all psstats_archive_* tables
...
; The list of periods that are available in the Psstats calendar
...
; Note: if you use any plugins, this need to be compliant with Psstats and
...
; when set to 1, all requests to Psstats will return a maintenance message without connecting to the DB
...
; this setting is used mainly to limit the amount of data that is stored by Psstats.
...
; Set this to 0 in case your Psstats database is very big, and suggested values may not appear in time
...
; default language to use in Psstats
default_language = en
...
; The date and period loaded by Psstats uses the defaults below. Possible values: yesterday, today.
...
; This setting is overridden in the UI, under "General Settings".
; The default value is to allow browsers to trigger the Psstats archiving process.
; This setting is only used if it hasn't been overridden via the UI yet, or if enable_general_settings_admin=0
enable_browser_archiving_triggering = 1

; By default, Psstats will force archiving of range periods from browser requests, even if enable_browser_archiving_triggering
...
; By default Psstats will automatically archive all date ranges any user has chosen in their account settings.
...
; By default Psstats runs OPTIMIZE TABLE SQL queries to free spaces after deleting some data.
; If your Psstats tracks millions of pages, the OPTIMIZE TABLE queries might run for hours (seen in "SHOW FULL PROCESSLIST \g")
; so you can disable these special queries here:
enable_sql_optimize_queries = 1

; By default Psstats is purging complete date range archives to free spaces after deleting some data.
...
; Psstats will check that usernames and password have a minimum length, and will check that characters are "allowed"
; This can be disabled, if for example you wish to import an existing User database in Psstats and your rules are less restrictive
disable_checks_usernames_attributes = 0

; Psstats will use the configured hash algorithm where possible.
...
; If set to 1, Psstats will automatically redirect all http:// requests to https://
; If SSL / https is not correctly configured on the server, this will break Psstats
; If you set this to 1, and your SSL configuration breaks later on, you can always edit this back to 0
; it is recommended for security reasons to always use Psstats over https
force_ssl = 0

; If set to 1 Psstats will prefer using SERVER_NAME variable over HTTP_HOST.
...
login_cookie_name = psstats_auth
...
; if specified, {DOMAIN} will be replaced by the current Psstats domain
...
; When configured, only users from a configured IP can log into your Psstats. You can define one or multiple
; IPv4, IPv6, and IP ranges. You may also define hostnames. However, resolving hostnames in each request
; may slightly slow down your Psstats.
...
; login_allowlist_ip[] = psstats.org
...
; By default when user logs out they are redirected to Psstats "homepage" usually the Login form.
; Uncomment the next line to set a URL to redirect the user to after they log out of Psstats.
...
; Set to 1 to allow using token_auths with write or admin access in iframes that embed Psstats.
...
language_cookie_name = psstats_lang
...
feedback_email_address = "feedback@psstats.org"
...
; during archiving, Psstats will limit the number of results recorded, for performance reasons
...
; by default, an update notification for a new version of Psstats is shown to every user. Set to 1 if only
; the superusers should see the notification.
show_update_notification_to_superusers_only = 0

; Set to 1 if you're using https on your Psstats server and Psstats can't detect it,
...
; Set to 1 if you're using more than one server for your Psstats installation. For example if you are using Psstats in a
...
; Psstats will determine the user IP by extracting the first IP address found in this proxy header.
;
...
; Whether to enable trusted host checking. This can be disabled if you're running Psstats
...
; If you use this Psstats instance over multiple hostnames, Psstats will need to know
; a unique instance_id for this instance, so that Psstats can serve the right custom logo and tmp/* assets,
; independently of the hostname Psstats is currently running under.
; instance_id = stats.example.com

; The API server is an essential part of the Psstats infrastructure/ecosystem to
; provide services to Psstats installations, e.g., getLatestVersion and
; subscribeNewsletter.
api_service_url = http://api.psstats.org
...
; When requesting report metadata with $period=range, Psstats needs to translate it to multiple periods for evolution graphs.
...
; Controls whether the user is able to upload a custom logo for their Psstats install
...
; absolute path to the chroot environment. eg. '/path/to/psstats/chrooted/'
absolute_chroot_path =

; The path (relative to the Psstats directory) in which Psstats temporary files are stored.
; Defaults to ./tmp (the tmp/ folder inside the Psstats directory)
tmp_path = "/tmp"

; The absolute path to a PHP binary file in case Psstats cannot detect your PHP binary. If async CLI archiving cannot be
...
; In some rare cases it may be useful to explicitly tell Psstats not to use LOAD DATA INFILE
...
; By setting this option to 1, it will be possible for Super Users to upload Psstats plugin ZIP archives directly in Psstats Administration.
; Enabling this opens a remote code execution vulnerability where
; an attacker who gained Super User access could execute custom PHP code in a Psstats plugin.
...
; - Trusted Psstats Hostname
enable_general_settings_admin = 1

; Disabling this will disable features like automatic updates for Psstats,
...
; This option defines the protocols Psstats's Http class is allowed to open.
...
; Comma separated list of plugin names for which console commands should be loaded (applies when Psstats is not installed yet)
...
; If set to 0 it will disable advertisements for providers of Professional Support for Psstats.
...
; The path to a custom cacert.pem file Psstats should use.
; By default Psstats uses a file extracted from the Firefox browser and provided here: https://curl.haxx.se/docs/caextract.html.
; The file contains root CAs and is used to determine if the chain of a SSL certificate is valid and it is safe to connect.
; Most users will not have to use a custom file here, but if you run your Psstats instance behind a proxy server/firewall that
...
; Disabling this feature can be useful for example when using the third party cookie, where all Psstats tracked sites
...
; Psstats uses "Privacy by default" model. When one of your users visit multiple of your websites tracked in this Psstats,
; Psstats will create for this user a fingerprint that will be different across the multiple websites.
...
; Psstats uses first party cookies by default. If set to 1,
; the visit ID cookie will be set on the Psstats server domain as well
...
; by default, the Psstats tracking cookie expires in 13 months (365 + 28 days)
; This is used only if use_third_party_id_cookie = 1
...
; after their last page view, it will be recorded as a new visit. In case you are using the Psstats JavaScript tracker to
...
; By default, Psstats will remove the most common parameters which are known to change often (eg. session ID parameters)
...
; if set to 1, Psstats attempts a "best guess" at the visitor's country of
...
; Scheduled tasks will only run if 'Enable Psstats Archiving from Browser' is enabled in the General Settings.
...
ignore_visits_cookie_name = psstats_ignore

; Comma separated list of variable names that will be read to define a Campaign name, for example CPC campaign
; Example: If a visitor first visits 'index.php?psstats_campaign=Adwords-CPC' then it will be counted as a campaign referrer named 'Adwords-CPC'
; Includes by default the GA style campaign parameters
campaign_var_name = "pk_cpn,pk_campaign,piwik_campaign,mtm_campaign,psstats_campaign,utm_campaign,utm_source,utm_medium"

; Comma separated list of variable names that will be read to track a Campaign Keyword
; Example: If a visitor first visits 'index.php?psstats_campaign=Adwords-CPC&psstats_kwd=My killer keyword' ;
...
campaign_keyword_var_name = "pk_kwd,pk_keyword,piwik_kwd,mtm_kwd,mtm_keyword,psstats_kwd,utm_term"
...
; ONLY CHANGE THIS VALUE WHEN YOU DO NOT USE PSSTATS ARCHIVING, SINCE THIS COULD CAUSE PARTIALLY MISSING ARCHIVE DATA
...
; Increasing this number can slow down the tracking in Psstats and put more load on the database.
...
; DO NOT USE THIS SETTING ON PUBLICLY AVAILABLE PSSTATS SERVER
; !!! Security risk: if set to 0, it would allow anyone to push data to Psstats with custom dates in the past/future and even with fake IPs!
...
; DO NOT USE THIS SETTING ON PUBLIC PSSTATS SERVERS
tracking_requests_require_authentication = 1

; By default, Psstats accepts only tracking requests for up to 1 day in the past. For tracking requests with a custom date
; date is older than 1 day, Psstats requires an authenticated tracking requests. By setting this config to another value
; You can change how far back Psstats will track your requests without authentication. The configured value is in seconds.
...
; If a value greater than 0 is configured, Psstats will configure MySQL with the set lock wait timeout in seconds during a
; tracking request. This can be useful if you have a high concurrency load on your server and want to reduce the time of
; lock wait times. For example configuring a value of 3-10 seconds may give your Psstats a performance boost if you have
...
; For a list of tracking parameters you can use on the left side view https://developer.psstats.org/api-reference/tracking-api
exclude_requests = ""
...
[Plugins]
; list of plugins (in order they will be loaded) that are activated by default in the Psstats platform
...
; This can be used to expose values from Psstats, to control for example a Mobile app tracking
SDK_batch_size = 10
