...
return array(
    ...
    'Psstats\Cache\Eager' => function (ContainerInterface $c) {
        $backend = $c->get('Psstats\Cache\Backend');
    ...
    'Psstats\Cache\Backend' => function (ContainerInterface $c) {
        ...
        if(!\Piwik\SettingsPiwik::isPsstatsInstalled()) {
            $backend = 'array';
        } else {
            try {
     ...
     * Thus it allows you to execute for example a newer version of Psstats with an older Psstats database version. Please note
     ...
     * you want to update Psstats without any outage when you know the current source code update will still run fine for a short time
